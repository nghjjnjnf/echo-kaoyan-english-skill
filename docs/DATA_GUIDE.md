...
公开基础知识库包含题面文本、题号映射、客观题答案、Echo 生成的翻译参考译文、翻译/完形整体难点评析与易错点总结；不包含逐题官方解析、作文范文或原始 Word 文件。
...
可将有权使用的考研词表放在 `references/vocabulary/`。建议使用 UTF-8 文本，每行一个词，文件名写清版本与来源。不要提交限制再分发的词表。

## 数据安全

- 原始 Word 默认被 `.gitignore` 排除。
- 提交前运行 `python scripts/validate_repo.py`，检查是否误加入原始 Word、本机路径或作文答案。
